...
We also tested Random Forest and LightGBM for comparison:
...
We tried splitting the problem into two stages: first a binary classifier to decide Normal vs Attack, then a separate classifier to identify which type of attack. In practice this scored 0.5729 — worse than the single model — because the binary classifier still struggled with R2L, and any R2L misclassified as Normal never even reached the second stage.

### Threshold Tuning

The most effective improvement came from adjusting the decision thresholds. By default, a model predicts whichever class has the highest probability. We modified this so that R2L and U2R had much lower thresholds, making the model more aggressive about detecting rare attacks at the cost of some precision. This pushed the score from 0.6240 to 0.6674.
...
The rows are actual classes, the columns are predicted classes. The diagonal shows correct predictions.

| Actual \ Predicted | DoS | Normal | Probe | R2L | U2R |
|--------------------|-----|--------|-------|-----|-----|
| **DoS** | **6024** | 981 | 72 | 383 | 0 |
| **Normal** | 76 | **9397** | 211 | 15 | 12 |
| **Probe** | 162 | 535 | **1646** | 68 | 10 |
| **R2L** | 0 | 1967 | 14 | **873** | 31 |
| **U2R** | 0 | 15 | 0 | 16 | **36** |

DoS and Normal are detected well. R2L remains the hardest class — 1967 out of 2885 R2L attacks are misclassified as Normal because many R2L subtypes in the test set do not appear in training at all. U2R improved significantly compared to the baseline, going from near-zero recall to 0.54 in our final model.
...
| Metric | Score |
|--------|-------|
| Cross-validation macro F1 (5-fold) | 0.9990 ± 0.0004 |
| Test macro F1 (KDDTest+) | 0.6674 |

There is a large gap between the cross-validation score and the test score. This is not a sign of overfitting — we never used the test set during training or tuning. The reason is that KDDTest+ deliberately contains attack subtypes that do not appear in KDDTrain+. Our cross-validation only sees the training distribution and scores very high as a result. When the model encounters genuinely new attack patterns in the test set, particularly in R2L and U2R, performance drops. This mirrors the real-world challenge where new attack variants constantly emerge and a model trained on known attacks cannot perfectly detect unknown ones.

---

## 5. What We Would Try Next

**Feature engineering.** We did not create any new features beyond what the dataset provides. Interaction features like the ratio of source to destination bytes, or combining login failure counts with connection flags, might help the model better distinguish R2L from Normal.

**Voting ensemble.** Combining XGBoost and LightGBM predictions through a majority vote sometimes catches what a single model misses. Since LightGBM scored 0.60 on its own, combining it with XGBoost might produce a better result than either alone.

**Per-subclass threshold tuning.** Right now we apply a single threshold for all R2L attacks. Identifying which R2L subtypes are most commonly confused with Normal and tuning thresholds more precisely for those could help further.

**Neural network.** An MLP classifier might learn more complex patterns, especially for the subtle differences between Normal and R2L traffic. We avoided it due to the additional complexity and tuning required, but it would be worth trying with more time.
